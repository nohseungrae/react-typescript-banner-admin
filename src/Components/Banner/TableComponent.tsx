...
const TableComponent: FunctionComponent<IProps> = ({
  ...
  const [removeReservedBanner, { data }] = useMutation(DELETE_RESERVEDBANNER, {
    update(cache: ApolloCache<any>, { data: { deleteReservedBannerByGraph } }) {
      ...
      const refetchData = temp?.splice(
        temp.findIndex((e: any) => e.id === id),
        1
      );
      console.log(refetchData, id, deleteReservedBannerByGraph?.id);
      const finalData = refetchData.map((item: any) =>
        item.reservedBanners.filter(
          (it: any) => it.id !== deleteReservedBannerByGraph.id
        )
      );
