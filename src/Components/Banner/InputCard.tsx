  ...
  const [addReservedBanner, { data: addReservedResult }] = useMutation(
    ADD_RESERVEDBANNER,
    {
      update(cache: ApolloCache<any>, { data: { addReservedBannerByGraph } }) {
        ...
        const banner = getNewBanners.filter(
          (item: any) => item.id === bannerIndex
        );
        const emptyArray = banner[0].reservedBanners;
        emptyArray.push(addReservedBannerByGraph);
        banner[0].reservedBanners = emptyArray;
        const newBanner = banner[0].reservedBanners;
        console.log(addReservedBannerByGraph, banner, newBanner);
        const finalData = getNewBanners.map((item: any) =>
          item.id === newBanner.id ? (item = newBanner) : item
        );
                  ...
                  <>
                    ...
                    <Group
                      ...
                      value={
                        bannerIndex ===
                        initialValues?.[key as keyof IBanners]?.id
                          ? initialValues?.[key as keyof IBanners]?.seq || 0
                          : 0
                      }
                      id="seq"
                      label="순서"
