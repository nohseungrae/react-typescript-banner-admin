...
const BannerLayout: React.FunctionComponent<IProps> = ({children, height, main, imgBoxWidth}) => {

    const BasicLayout: React.FunctionComponent<IProps> = ({children, height}) => {
        return (
            <SS.Core.Row>
                <SS.Core.Col style={{
                    overflow: "hidden",
                    borderRadius: "5px",
                    maxHeight: `calc(${height} + 22px)`,
                }}>
                    <Text>
                        이벤트 사진<SS.Core.Button padding={"2px"} margin={"0 0 0 5px"}
                                              theme={theme.pink}>적용중</SS.Core.Button>
                    </Text>
                    <SS.Core.Col style={{height: height, width: imgBoxWidth ? imgBoxWidth : "100%"}}>
    ...
    return (
        <SS.Core.Container>
            <SS.Core.Inner>
                {main ?
                    <BasicLayout height={height} children={children}/>
                    :
                    <BasicLayout height={height} children={children}/>
                }
            </SS.Core.Inner>
