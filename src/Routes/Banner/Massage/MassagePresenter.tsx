    ...
    return (
        <BannerLayout banner={banner?.massage} name={"스파&마사지"} height={"435px"} children={
            <>
                ...
                <InputCard variables={variables} banner={banner} bannerIndex={banner?.massage?.id}
                           uploadHeight={"435px"} maxWidth={930}/>
                <ReserveComponent id={banner?.massage?.id} variables={variables}
                                  reservedBanners={banner?.massage?.reservedBanners}/>
            </>
        }/>
    );
};
