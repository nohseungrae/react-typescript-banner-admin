    ...
    return (
        <BannerLayout banner={banner?.makeUp} name={"메이크업&화장"} height={"435px"} children={
            <>
                ...
                <InputCard variables={variables} banner={banner} bannerIndex={banner?.makeUp?.id} uploadHeight={"435px"}
                           maxWidth={930}/>
                <ReserveComponent variables={variables} id={banner?.makeUp?.id}
                                  reservedBanners={banner?.makeUp?.reservedBanners}/>
            </>
        }/>
    );
};
