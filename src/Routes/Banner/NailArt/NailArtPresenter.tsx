    ...
    return (
        <BannerLayout banner={banner.nailArt} name={"네일아트"} height={"435px"} children={
            <>
                ...
                <InputCard variables={variables} banner={banner} bannerIndex={banner?.nailArt?.id}
                           uploadHeight={"435px"} maxWidth={930}/>
                <ReserveComponent id={banner?.nailArt?.id} variables={variables}
                                  reservedBanners={banner?.nailArt?.reservedBanners}/>
            </>
        }/>
    );
};
